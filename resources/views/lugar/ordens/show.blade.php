            ...
            <div style="margin: 20px;">
                <div class="flex flex-row justify-between">
                    <h2 class="mt-6 text-xl font-semibold text-gray-900 dark:text-white">Nueva orden para la mesa #
                        {{ \App\Models\Lugar\NotaVenta::find($orden->IDVenta)->NroMesa }}</h2>
                    ...
                    <table style="width: calc(100% - 32px); margin: 16px;">
                        ...
                        @foreach(\App\Models\Lugar\Ordena::all() as $ordena)
                            @if($ordena->NumOrden == $orden->NumOrden)
                                    <?php
                                    $producto = \App\Models\Inventario\Producto::find($ordena->CodProd);
                                    $item = \App\Models\Inventario\Item::find($producto->CodProd);
                ...
                <div class="flex flex-row justify-between">
                    ...
                    <div class="mt-4 space-x-2">
                        <a href="{{ route('nota_ventas.show', \App\Models\Lugar\NotaVenta::find($orden->IDVenta)) }}">
