@php use App\Models\Inventario\Item;use App\Models\Inventario\ProdEntrante;use App\Models\Inventario\Producto; @endphp
<x-app-layout>
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <div class="mt-6">
            <div style="margin: 20px;">
                <div class="flex flex-row justify-between">
                    ...
                    <form method="GET" action="{{ route('productos.index') }}">
                        <x-primary-button class="mt-4"
                                          style="margin-right: 2px">{{ __('PRODUCTOS') }}</x-primary-button>
                    </form>
                ...
                <div class="mt-6 bg-white rounded-lg divide-y">
                    ...
                    <table style="width: calc(100% - 32px); margin: 16px;">
                        ...
                        @foreach($items as $item)
                            @php
                                $producto = Producto::find($item->CodItem);
                                $pr_ent = ProdEntrante::where("CodItem", "=", $item->CodItem)->OrderBy('NroCompra', "Desc")->first();
                            @endphp
                            <tr>
                                <td style="flex-grow: 4">{{ $item->Nombre }}</td>
                                <td style="flex-grow: 4">{{ is_null($producto) ? "---" : number_format($producto->Precio, 2) }}</td>
                                ...
                                <td>
                                    <x-dropdown>
                                        ...
                                        <x-slot name="content">
                                            <x-dropdown-link :href="route('item.edit', $item)">
                                                {{ __('Editar') }}
                                            </x-dropdown-link>
                                            <x-dropdown-link :href="route('nota_salida.create', ['CodItem' => $item->CodItem])">
                                                {{ __('Registrar descarte') }}
                                            </x-dropdown-link>
                                        </x-slot>
                                    </x-dropdown>
                                </td>
                ...
                <form method="GET" action="{{ route('nota_salida.index') }}">
                    <x-primary-button class="mt-4"
                                      style="margin-right: 2px">{{ __('NOTAS DE SALIDA') }}</x-primary-button>
                </form>
            </div>
        </div>
